...
/*声明线程运行服务程序*/
static void *pthread_add2(void *arg);
static void *pthread_add3(void *arg);

static pid_t get_lwp_id(void) {
...
int main (void)
{
 /*线程的标识符*/
  ...
  /*分别创建线程1、2*/
  ret = pthread_create( &pt_1, NULL, pthread_add2, NULL);  
  ret = pthread_create( &pt_2, NULL, pthread_add3, NULL); 
  /*等待线程1、2的结束*/
...
static void *pthread_add2(void *arg)
{
  int i = 0;
  (void)arg;
  printf ("This is pthread_1!PID = %d,LWP = %d,tid = %lu\n",getpid(),get_lwp_id(),(unsigned long)pthread_self());
  for( i=0; i<3; i++ ){
